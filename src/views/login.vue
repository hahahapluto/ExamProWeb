  ...
  formRef.value.validate(async valid => {
    ...
    if (loginServe) {
      ...
      if (isSuccess) {
        ...
        if (changeState.confirmName === '注册') {
          sessionSaveData('role', form.role)
        } else {
          sessionSaveData('role', loginData.data.role)
        }
...
<template>
  <div class="login">
    <div class="login-box" :class="{ regist_height: changeState.isRegister }">
      ...
      <!-- 登录表单区域 -->
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" class="login-box-form" autocomplete="off">
        <el-form-item label="用&nbsp;&nbsp;户&nbsp;&nbsp;名：" prop="username">
          <ElInput placeholder="请输入用户名" v-model="form.username" input-style="height:35px;" autocomplete="off"></ElInput>
        </el-form-item>
        <el-form-item label="密&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;码：" prop="password">
          <ElInput placeholder="请输入密码" v-model="form.password" input-style="height:35px;" show-password type="password" autocomplete="off"></ElInput>
        </el-form-item>
        <el-form-item label="确认密码：" v-if="changeState.isRegister" prop="checkPassword">
          <ElInput placeholder="请再次输入密码" v-model="form.checkPassword" input-style="height:35px;" show-password type="password" autocomplete="off"></ElInput>
        </el-form-item>
        ...
        <el-form-item class="login-box-form-btns">
          <button type="button" class="confirm" @click="confirm($router)">立即{{ changeState.confirmName }}</button>
          <div class="change" @click="changeModel()">去{{ changeState.changeName }}</div>
        </el-form-item>
      </el-form>
    </div>
  </div>
