...
onMounted(() => {
  counterStore.getmyQuestionList()
})

const counterStore = mainStore()
const { myQuestionList } = storeToRefs(counterStore)
 ...
 *  return allTableData.filter((item: Ques) => {
    // 由于设置的0-2对应的不同 则需要转化成数值-1再转成字符串
    return item.type == (parseInt(searchData) - 1).toString()
  })
 */
...
function getAllQuestion(): any { throw new Error('Function not implemented.') } function cloneDeep(allTableDate: any): any { throw new Error("Function not implemented."); } function cloneDeep(allTableDate: any): any { throw new Error("Function not implemented."); }
