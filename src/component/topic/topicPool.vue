...
const AllQuestion = async (pageCount: number, pageSize: number) => {
  allTableData = (await getQuesPass(pageCount, pageSize)).data.data;
...
const getQuestionPoolPageCountData = async () => {
  let res = await getQuestionPoolPageCount(10);
  console.log(res);
...
const onCurrentPageChange = (newPage: number) => {
  ...
  AllQuestion(currentPage.value, pageSize.value);
};
...
onMounted(() => {
  AllQuestion(currentPage.value, pageSize.value);
  getBankData();
  getQuestionPoolPageCountData();
});
</script>

<template>
  <div>
    <h1 style="height: 45px">题目池</h1>
    <!-- 筛选试题信息 -->
    <div class="selector">
              ...
              <div class="des">
                ...
                <div class="des-right">
                  <p class="des-right-pp">
                    {{ getQues(props.row.questionDescription) }}
                  </p>
                  <p
                    v-if="props.row.questionType !== '主观题'"
                    class="des-right-p"
                  >
                    A、 {{ getOption("A=", props.row.questionDescription) }}
                  </p>
                  <p
                    v-if="props.row.questionType !== '主观题'"
                    class="des-right-p"
                  >
                    B、 {{ getOption("B=", props.row.questionDescription) }}
                  </p>
                  <p
                    v-if="props.row.questionType !== '主观题'"
                    class="des-right-p"
                  >
                    C、 {{ getOption("C=", props.row.questionDescription) }}
                  </p>
                  <p
                    v-if="props.row.questionType !== '主观题'"
                    class="des-right-p"
                  >
                    D、 {{ getOption("D=", props.row.questionDescription) }}
                  </p>
                </div>
              </div>
              <div class="answ">
                ...
                <div class="answ-box">
                  ...
                  <div class="answ-box-right">
                    <p class="answ-box-right-p">
                      {{ props.row.questionAnswer }}
                    </p>
                  </div>
                </div>
              </div>
