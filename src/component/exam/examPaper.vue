...
import { addProctoringRecord, findProctorsByExamID, getProctoringRecords } from '../../request/api/invigilate/invigilate'
...
const sendMsg = async () => {
  ...
  if (issueContent != '') {
    ...
    const res = await addProctoringRecord(examineeID, examID, proctorID, issueContent, examineeID)
    // 发送成功的话
    if (!res.data.code) {
...
<template>
  <el-container v-if="!paperData.showFlag">
    ...
    <el-aside width="280px" class="quesSequenceBox">
      ...
      <el-button @click="finishPaper" v-if="sessionGetData('role') != 3 && sessionGetData('role') != 2 && sessionGetData('role') != 1">交卷</el-button>
    </el-aside>
    <el-main class="createPaper">
      <div class="createPaper-info">
        <div class="createPaper-paperName">
          <h1>{{ paperData.paperName }}</h1>
          <el-popover placement="right" :width="700" trigger="click" @hide="popoverHiddenHandler" @show="showPopover">
            <template #reference>
              <div class="right" @click="getProctoringRecordsData">
                <svg t="1667993211879" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2461" width="128" height="128">
                  <path
              ...
              <div class="InvigilationChat-container">
                ...
                <div class="chat_message_box">
                  <div class="first">
                    ...
                    <div class="right">
                      <svg t="1666856821084" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2805" width="200" height="200">
                        <path
                    ...
                    <div class="container" ref="chatBox">
                      <div
                        ...
                        <div class="message">
                          <div class="left">
                            <a href="">
                              <img v-if="item.proctorID != item.senderID" src="../../assets/anchor-1.png" alt="" />
                              <img v-if="item.proctorID == item.senderID" src="../../assets/teather.png" alt="" />
                            </a>
                          </div>
                          <div class="right">
                  ...
                  <div class="send_bottom">
                    <div class="container">
                      <div class="set">
                        <div class="icon">
                          <svg t="1666858230685" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2264" width="200" height="200">
                            <path d="M954.06592 738.9952a215.07072 215.07072 0 1 0-215.07072 215.07072 215.07072 215.07072 0 0 0 215.07072-215.07072z" fill="#F4CA1C" p-id="2265"></path>
                            <path
                        ...
                        <div class="icon">
                          <svg t="1666858365842" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2433" width="200" height="200">
                            <path
                        ...
                        <div class="icon">
                          <svg t="1666858497283" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="7426" width="200" height="200">
                            <path
                              d="M820.8896 807.9872H487.46496a39.64928 39.64928 0 0 0-22.58944 7.07072l-135.75168 94.13632a39.7056 39.7056 0 0 1-61.39392-24.19712l-10.10176-45.74208a39.7824 39.7824 0 0 0-38.80448-31.26784l0.19968-148.992a146.5344 146.5344 0 0 1 146.24256-146.62656h535.1168a39.82848 39.82848 0 0 1 39.74656 39.90528v136.00768a119.4752 119.4752 0 0 1-119.23968 119.7056z"
                              fill="#F4CA1C"
                              p-id="7427"></path>
                      ...
                      <div class="last_textarea">
                        <textarea name="" ref="last_textarea" cols="30" rows="10"></textarea>
