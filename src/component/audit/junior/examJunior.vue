...
import { getAllExam } from "../../../request/api/paper/exam";
...
const passQues = async (scope: any) => {
  ...
  const { code, msg } = (await setExamJuniorPass(scope.examID)).data;
...
const refuseQues = async (scope: any) => {
  console.log(scope.examID);
  const { code, msg } = (await setExamJuniorRefuse(scope.examID)).data;
  if (code == 0) ElMessage.success(msg);
  getExamList();
};
</script>
